...
/*
      if (options.where && !options.group && options.model) {
        options.group = '\'' + (options.tableAs || options.model.name) + '.' + options.model.primaryKeyField + '\'';
        ...
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (options.includeIgnoreAttributes !== false) {
            ...
            if (include.subQuery && subQuery) {
              ...
              subQueryAttributes = subQueryAttributes.concat(mainAttributes);
            } else {
              mainAttributes = mainAttributes.concat(attributes);
            }
          }
          ...
          if (through) {
            ...
            if (include.where || include.through.where || mainWhereHasAssociation) {
              if (include.where) {
                ...
                if (targetWhere) {
                  if (!include.required) {
                    ...
                    targetWhere = {};
                    targetWhere[as] = include.where;

                    options.where = options.where || {};
                    if (options.where instanceof Utils.and) {
                      options.where.args.push(targetWhere);
                    } else if (Utils._.isPlainObject(options.where)) {
                      options.where[as] = include.where;
                    } else {
                      options.where = new Utils.and(options.where, targetWhere);
                    }
                  }
                }
              }
            ...
            if (include.where || mainWhereHasAssociation) {
              targetWhere = self.getWhereConditions(include.where, self.sequelize.literal(self.quoteIdentifier(as)), include.model, whereOptions);
              if (targetWhere) {
          ...
          if (include.include) {
            include.include.forEach(function(childInclude) {
              ...
              // subQueryAttributes is recklessly passed around through functions and changed
              // this should be reviewed
              var thisSubQueryAttributes = subQueryAttributes;
              var childJoinQueries = generateJoinQueries(childInclude, as);
              //joinQueries.mainQuery = joinQueries.mainQuery.concat(joinQueries.subQuery);

              if (childInclude.subQuery && subQuery || childInclude.required === false && subQuery) {
                subQueryAttributes = subQueryAttributes.concat(attributes);
                //subQueryAttributes = thisSubQueryAttributes.concat(attributes).concat(_.xor(subQueryAttributes, thisSubQueryAttributes));
                if (throughAttributes) {
                  subQueryAttributes = subQueryAttributes.concat(throughAttributes);
                }
                joinQueries.subQuery = joinQueries.mainQuery.concat(childJoinQueries.subQuery);
              }
              if (childJoinQueries.mainQuery) {
                joinQueries.mainQuery = joinQueries.mainQuery.concat(childJoinQueries.mainQuery);
              }
              if (childJoinQueries.subQuery) {
                //joinQueries.mainQuery = joinQueries.mainQuery.concat(childJoinQueries.subQuery);
              }
          ...
          //joinQueries.subQuery =  _.union(joinQueries.mainQuery, joinQueries.subQuery);
      ...
      if (subQuery) {
        //subQueryItems.push('SELECT ' + mainAttributes.join(', ') + ' FROM ' + options.table);
        var selectTable = mainTableAs ? mainTableAs : options.table;
        subQueryItems.push('SELECT ' + selectTable + '.' + subQueryAttributes[0].split('.')[1] + ' FROM ' + options.table)
        if (mainTableAs) {
      ...
      if (options.hasOwnProperty('where')) {
        ...
        if (options.where) {
          if (subQuery) {
            ...
            if (!options.group) {
              options.group = this.quoteTable(model.name) + '.' + this.quoteIdentifier(model.primaryKeyField);
            }
          } else {
      ...
      if (options.hasIncludeWhere && !options.group) {
        options.group = this.quoteTable(model.name) + '.' + this.quoteIdentifier(model.primaryKeyField);
      }
      ...
      if (subQuery) {/*
        ...
        /**/
        query = 'SELECT ' + mainAttributes.join(', ');
        query += ' FROM ' + options.table;
        if (mainTableAs) {
          query += ' AS ' + mainTableAs;
        }
        query += mainJoinQueries.join('');
        query += ' WHERE ' + mainTableAs + '.' + '`id`' + ' IN (';
        query += subQueryItems.join('');
        query += ') ';
        query += mainQueryItems.join('');
        /**/
    ...
    whereItemsQuery: function(where, options, binding) {
      ...
      if (_.isPlainObject(where)) {
        _.forOwn(where, function (value, key) {
          ...
          var expandAssociationWhere = function(value, key) {
            if (_.contains(options.expandedAssociationList, key)) {
              _.forOwn(value, function(nestedValue, nestedKey) {
                key = key + '.' + nestedKey;
                value  = nestedValue;
                var nestedExpansion = expandAssociationWhere(value, key);
                key = nestedExpansion[0];
                value = nestedExpansion[1];
              });
            }
            return [key, value];
          };

          if (options && options.expandedAssociationList) {
            var expandedAssociationWhere = expandAssociationWhere(value, key);
            key = expandedAssociationWhere[0];
            value = expandedAssociationWhere[1];
          }
    ...
    whereItemQuery: function(key, value, options) {
      ...
      var isAssociatedKey = _.contains(options.expandedAssociationList, key.split('.')[0]);

      var keyParts = key.split('.');
      if (keyParts.length > 1) {
        var lastIdentifier = this.quoteIdentifier(keyParts.pop());
        key = this.quoteIdentifier(keyParts.join('.')) + '.' + lastIdentifier;
      } else {
        key = this.quoteIdentifier(key);
      }

      if (options.prefix && !isAssociatedKey) {
    ...
    */
    getWhereConditions: function(smth, tableName, factory, options, prepend) {
      ...
      options = options || {};
      var expandedAssociationList = options.expandedAssociationList || [];

      var expandAssociationWhere = function(options, parent) {
        parent = parent || '';
        var list = [];
        if (options) {
          _.each(options.include, function(include) {
            list.push(parent + include.as);
            if (include.through) {
              list.push(parent + include.through.model.tableName);
            }
            if (include.include) {
              list = list.concat(expandAssociationWhere(include, include.as + '.'));
            }
          });
        }
        return list;
      };
      var expandedAssociationList = expandAssociationWhere(options);
      ...
      if (smth && smth._isSequelizeMethod === true) { // Checking a property is cheaper than a lot of instanceof calls
        if (smth instanceof Utils.and || smth instanceof Utils.and) {
          return self.whereItemsQuery(smth, {
            ...
            expandedAssociationList: expandedAssociationList
          });
        }
